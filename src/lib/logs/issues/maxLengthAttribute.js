...
@author      : Riya
@date        : 2023-03-21
@description : This function sets the data object if max length attribute is not present in input tag.
...
const main = (issueFileGenericeObj, issueItem) => {
  ...
  if (filterStr.test(fileContent)) {
    ...
    const maxLengthIssueArray = issueArray.filter((i) => !i.includes('maxlength='));
    if (maxLengthIssueArray && maxLengthIssueArray.length > 0) {
      ...
      issueFoundObj.issueCount = maxLengthIssueArray.length;
      /* iterate over the issue array and call the get line number function */
      maxLengthIssueArray.forEach((element) => {
